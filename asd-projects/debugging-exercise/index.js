$(document).ready(function () {
  ...
  function update() {
    ...
    for (var i = 0; i < maxGhosts; i++) {
      ...
      // ~~ log info for the first ghost
      if (i === 0) {
        console.log("info for first ghost");
        console.log(ghost);
      }
  ...
  function moveGhost(ghost) {
    //ghost.x = ghost.speedX; // ~ ~ I think this is an error.  Should be += instead of =
  ...
  function bounceGhost(ghost) {
    console.log("inside of bounceGhost()");
    console.log("width = ", boardWidth, "height = ", boardHeight);
    ...
    // this bounces off the BOTTOM wall
      ...
      ghost.y -= ghost.speedY;
      //ghost.speedX *= -1;             ~ ~ need to update speedY, NOT speedX !!!
      ghost.speedY *= -1;
    }
  }
  ...
  function updateGhostOnScreen(ghost) {
    //maxGhosts = 0;   ~ ~ There is NO REASON to set maxGhosts to ZERO !
